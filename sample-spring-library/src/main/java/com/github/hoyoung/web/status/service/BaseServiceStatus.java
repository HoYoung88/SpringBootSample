 ...
 */
@Getter
public enum BaseServiceStatus implements ServiceStatus {
  SUCCESS("200", "요청이 정상적으로 처리되었습니다."),
  NOT_FOUND("-404", "요청하신 정보를 찾을 수 없습니다."),
  BAD_REQUEST("-400", "올바르지 않는 요청입니다."),
  UNAUTHORIZED("-401", "로그인 정보가 존재하지 않습니다."),
  METHOD_NOT_ALLOWED("-405", "요청한 Method[%s]는 허용되지 않는 Method입니다."),
  NOT_ACCEPTABLE("-406", "허용되지 않는 콘텐츠 특성입니다."),
  UNSUPPORTED_MEDIA_TYPE("-415", "요청한 Media Type[%s]은 지원하지 않는 Media Type입니다."),
  ...
  BaseServiceStatus(final String code, final String message) {
    this.code = code;
